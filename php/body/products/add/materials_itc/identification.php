...
if(
    isset(
            ...
            */
            ...
            ========================= ABREVIATION =========================
            */
              $mat_itc_abrev = substr($Name,0,5);
              $mat_itc_abrev = strtoupper($mat_itc_abrev);
  
            // COUNT NOMBRE D'ABREVIATION
              $check_nbr_mat_itc_abrev= mysqli_query($dbc,"SELECT * FROM `mat_itc_id` WHERE `mat_itc_abrev` ='$mat_itc_abrev'");
              $checkrows_nbr_med_mat_itc_abrev=mysqli_num_rows($check_nbr_mat_itc_abrev);
  
              if($checkrows_nbr_med_mat_itc_abrev > 0)
              {$mat_itc_nbr_abrev = $checkrows_nbr_med_mat_itc_abrev + 1;}
              else 
              { $mat_itc_nbr_abrev = 1;}
            ...
            */
            //NOMBRE DE CARACTER DANS LE COUNT DE L'ABREVIATION
              $nb_char = strlen($mat_itc_nbr_abrev);
              switch($nb_char){
                  case 1 :
                      $nb_chars = "-00";
                      break;
          ...
          // COUNT NOMBRE NAME
            $sql_Name_check = mysqli_query($dbc,"SELECT * FROM `mat_itc_id` WHERE `name_check` ='$Name_check'");
          ...
          */
          //NOMBRE DE CARACTER DANS LE COUNT DE L'ABREVIATION
            ...
            // ID CODE MATERIALS ITC
              $id_code_mat_itc= "MAT-ITC" . "-" . $mat_itc_abrev . $nb_chars . $mat_itc_nbr_abrev;
            ...
            */
            $check_mat_seral_number= mysqli_query($dbc,"SELECT * FROM `mat_itc_id` WHERE `sereal_number` ='$sereal_number' AND `name_check` ='$Name_check'");
            ...
            else
            {
              ...
              $sql_mat_itc_id_adhesive = "INSERT INTO `mat_itc_id` (
                /*ID PRIMARY*/
                `id`, 
                /*CREATE*/
                `created_date`, 
                /*ID ABREV*/
                `mat_itc_abrev`, `mat_itc_nbr_abrev`, `id_code_mat_itc`,
                /*NAME*/
                `Descriptions`, 
                 /*NAME CHECK*/
                 `name_check`, 
              ...
              $query_mat_itc_id_adhesive = $db->prepare($sql_mat_itc_id_adhesive);
              //On execute la requete
             $query_mat_itc_id_adhesive->execute();
            
             //On recupere l'id de l'article
              $mat_itc_id = $db->lastInsertId();
