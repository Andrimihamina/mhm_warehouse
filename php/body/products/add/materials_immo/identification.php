...
if(
    isset(
            ...
            */
            ...
            ========================= ABREVIATION =========================
            */
              $mat_imo_abrev = substr($Name,0,5);
              $mat_imo_abrev = strtoupper($mat_imo_abrev);
  
            // COUNT NOMBRE D'ABREVIATION
              $check_nbr_mat_imo_abrev= mysqli_query($dbc,"SELECT * FROM `mat_imo_id` WHERE `mat_imo_abrev` ='$mat_imo_abrev'");
              $checkrows_nbr_med_mat_imo_abrev=mysqli_num_rows($check_nbr_mat_imo_abrev);
  
              if($checkrows_nbr_med_mat_imo_abrev > 0)
              {$mat_imo_nbr_abrev = $checkrows_nbr_med_mat_imo_abrev + 1;}
              else 
              { $mat_imo_nbr_abrev = 1;}
            ...
            */
            //NOMBRE DE CARACTER DANS LE COUNT DE L'ABREVIATION
              $nb_char = strlen($mat_imo_nbr_abrev);
              switch($nb_char){
                  case 1 :
                      $nb_chars = "-00";
                      break;
          ...
          // COUNT NOMBRE NAME
            $sql_Name_check = mysqli_query($dbc,"SELECT * FROM `mat_imo_id` WHERE `name_check` ='$Name_check'");
          ...
          */
          //NOMBRE DE CARACTER DANS LE COUNT DE L'ABREVIATION
            ...
            // ID CODE MATERIALS 
              $id_code_mat_imo= "MAT-IMO" . "-" . $mat_imo_abrev . $nb_chars . $mat_imo_nbr_abrev;
            ...
            */
            $check_mat_seral_number= mysqli_query($dbc,"SELECT * FROM `mat_imo_id` WHERE `sereal_number` ='$sereal_number' AND `name_check` ='$Name_check'");
            ...
            else
            {
              ...
              $sql_mat_imo_id_adhesive = "INSERT INTO `mat_imo_id` (
                /*ID PRIMARY*/
                `id`, 
                /*CREATE*/
                `created_date`, 
                /*ID ABREV*/
                `mat_imo_abrev`, `mat_imo_nbr_abrev`, `id_code_mat_imo`,
                /*NAME*/
                `Descriptions`, 
                 /*NAME CHECK*/
                 `name_check`, 
              ...
              $query_mat_imo_id_adhesive = $db->prepare($sql_mat_imo_id_adhesive);
              //On execute la requete
             $query_mat_imo_id_adhesive->execute();
            
             //On recupere l'id de l'article
              $mat_imo_id = $db->lastInsertId();
