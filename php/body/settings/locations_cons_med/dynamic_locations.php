...
if(!empty($_POST)){
    ...
    if(
        ...
        && !empty($_POST["location_definitions"]) && !empty($_POST["location_descriptions"])
            ...
            $check_locations= mysqli_query($dbc,"SELECT * FROM `cos_location` WHERE `location_descriptions` ='$location_descriptions'");
            ...
            //On ecrit la requete
            $sql_settings_location= "INSERT INTO `cos_location` (`location_descriptions`, `location_definitions`) VALUES ('$location_descriptions', '$location_definitions')";
